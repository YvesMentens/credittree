credittree <- function(formula, data, subset, na.action, weights, control = credittree.control(...), ...) 
{
  ...
  finalPerformance <- 0
  performances <- array(999999, control$niterations)
  ...
  tree <- function(nInstances, nVariables, varType, ndata,
                   ...
                   splitV, splitP, csplit, maxNode, control,performances) {
    on.exit(.C("freememory", PACKAGE = "credittree"))
    out <- .C("tree", PACKAGE = "credittree",
              ...
              as.double(control$p0),
              as.double(control$p1),
              as.double(control$ROI),
              as.double(control$lambda),
              as.logical(control$verbose),
              as.double(control$seed),
              as.double(performances))
    return(out)
  }
