...
extern "C"{
  ...
  void tree(int* nInst, int* nVar, int* varType, double* nData, int* weights,
            ...
            int* method, double* p0, double* p1, double* ROI,
            double* lambda, bool* verbose, double* seed, double* performances){
    container = new Container(nInst, nVar, varType, nData, weights,
                              ...
                              method, p0,p1,ROI,lambda, verbose, seed, performances);
  }//tree
...
static const R_CMethodDef CEntries[] = {
  ...
  {"tree",       (DL_FUNC) &tree,       30}, // change to 31 when using 'fitnesscurve'
  {NULL, NULL, 0}
};

void R_init_credittree(DllInfo *dll)
{
...
Container::Container(int* R_nInstances, int* R_nVariables, int *R_varType, double* R_nData, int* R_weights,
                     ...
                     int* R_method, double* R_p0, double* R_p1, double* R_ROI,double* R_lambda, bool* R_verbose, double* R_seed,double* R_performances){
  // constructor
  ...
  this->performanceHistory = new double[1000];
  ...
  this->performances = new double[this->nIterations];
  ...
  for (int i = 0; i < this->nInstances; i++){
    ...
    //this->performances[i] = R_performances[i];
  }
    ...
    if(succ == true){
      if(this->elitismList[0] < this->nTrees){
        ...
        if(this->verbose == TRUE){
          Rcout << "niterations = " << *R_nIterations << "\n";
         // std::cout << "\n";
        }
        ...
        *R_finalPerformance = this->trees[this->elitismList[0]]->performance;
        for(int i = 0; i < *R_nIterations; i++){
          Rcout << i;  
          R_performances[i] = this->performances[i];    
        }
        //*R_performances = this->performances
      } // if elitism
...
bool Container::evolution(){
  ...
  for(int i = 0; i < this->nIterations; i++){
    ...
    if(this->verbose == TRUE && i > 0 && (i+1)%10 == 0){
      Rcout << (i+1) << "th iteration \n";
    }
    ...
    //if(i>0) {
      //Rcout << "\nTest " << this->trees[this->elitismList[0]]->performance;
      //R_performances[i] = this->trees[this->elitismList[0]]->performance;
      //Rcout << "\n" << this->performances[i];
     
      //Rcout << "\n" << this->performances[i];
    //}
    //Rcout << "Test2";
    if(this->verbose == TRUE && i > 0 && (i+1)%5 == 0){
      Rcout << "Perf. = " << this->trees[this->elitismList[0]]->performance << "\n";
      this->performances[i] = this->trees[this->elitismList[0]]->performance;
...
bool Container::evaluateTree(int treeNumber, bool pruneIfInvalid, int nodeNumber){
  ...
  this->trees[treeNumber]->calculateTotalCosts(this->method, this->p0, this->p1, this->ROI,this->lambda, this->sumWeights);
  return true;
} // end evaluateTree
